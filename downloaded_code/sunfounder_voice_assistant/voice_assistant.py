...
class VoiceAssistant:
    ...
    Args:
        ...
        round_cooldown (float, optional): Seconds to wait between conversation rounds, default is 1.0
    """
    ...
    def main(self) -> None:
        ...
        while self.running:
            ...
            # Check if any trigger wants exclusive microphone access
            # (e.g., conversation mode that handles its own listening)
            exclusive_trigger_active = False
            ...
            # Start listening wake words if wake enabled AND no exclusive trigger active
            if self.wake_enable and not exclusive_trigger_active:
            ...
            # Stop listening wake words if wake enabled AND was started
            if self.wake_enable and not exclusive_trigger_active:
                self.stt.stop_listening()
            ...
            if response_text != '':
                ...
                self.after_say(response_text)

            # on finish a round
            self.on_finish_a_round()

            # Wait before next round (configurable cooldown)
